...
function Favorities() {
  ...
  useEffect(() => {
      ...
      axios
        .get(`http://localhost:5173/favorities`)
        .then((response) => {
  ...
  const removeFromFavorites = async (itemId) => {
    try {
      ...
      await axios.delete(`http://localhost:5173/favorities/${selectedItem.id}`);
      // Refresh the wishlist items
      const response = await axios.get('http://localhost:5173/favorities');
